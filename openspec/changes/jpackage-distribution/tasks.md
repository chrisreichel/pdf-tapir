...
- [x] 1.1 Add `stage.getIcons().add(new Image(getClass().getResourceAsStream("/ICON.png")));` to `PdfEscrotoApp.start()` before `stage.show()`
...
- [x] 4.8 Add step: run `jpackage` with `--input target/lib/`, `--main-jar pdf-escroto-$VERSION.jar`, `--main-class com.pdfescroto.PdfEscrotoApp`, `--name "PDF Escroto"`, `--app-version $VERSION`, `--icon ${{ matrix.icon }}`, `--type ${{ matrix.jpackage-type }}`
